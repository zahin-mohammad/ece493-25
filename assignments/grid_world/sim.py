...
def value_iteration(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Value Iteration task {task+1}")
...
def policy_iteration(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Policy Iteration task {task+1}")
...
def sarsa_learning(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Sarsa Learning task {task+1}")
...
def expected_sarsa_learning(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Expected Sarsa Learning task {task+1}")
    rl = ExpectedSarsaLearning(list(range(env.n_actions)))
    do_algorithm(rl, task, env)


def q_learning(task):
    env = Maze(agentXY, goalXY, tasks[task][0],
               tasks[task][1], title=f"Q Learning task {task+1}")
...
def double_q_learning(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Double Q Learning task {task+1}")
...
def eligibility_trace_learning(task):
    env = Maze(agentXY, goalXY, tasks[task][0], tasks[task]
               [1], title=f"Eligibility Trace Learning task {task+1}")
