...
from RL_brain_VI import ValueIteration as rlalg2
from RL_brain_PI import PolicyIteration as rlalg3
from RL_brain_sarsa import SarsaLearning as rlalg4
from RL_brain_qlearning import QLearning as rlalg5
...
def plot_rewards(task_experiments):
    ...
    for task, experiments in task_experiments.items():
        for i, (env,RL, data) in enumerate(experiments):
            ...
            axs[axs_keys[task][0], axs_keys[task][1]].plot(x_values, y_values, c=color_list[i], label=label_list[-1])
    ...
    fig.text(0.04, 0.5, 'Rewards', va='center', rotation='vertical', fontsize=18)
...
def plot_individual_plots(task_experiments):
    ...
    for j, name in enumerate(name_to_tasks.keys()):
        for i, item in enumerate(name_to_tasks[name].items()):
            ...
            y_values=data['global_reward']
            axs[axs_keys[j][0], axs_keys[j][1]].plot(x_values, y_values, c=color_list[i], label=label_list[-1])
...
def plot_time(task_experiments):
    ...
    fig.text(0.04, 0.5, 'Time [ms]', va='center', rotation='vertical', fontsize=18)
...
def update(task_num, env, RL, data, episodes=50):
...
if __name__ == "__main__":
    ...
    def rl_brain_vi(task):
        env = Maze(agentXY, goalXY, tasks[task][0], tasks[task][0])
        rl = rlalg2(list(range(env.n_actions)), env)
        data = {}
    ...
    def rl_brain_pi(task):
        env = Maze(agentXY, goalXY, tasks[task][0], tasks[task][0])
        rl = rlalg3(list(range(env.n_actions)), env)
        data = {}
        env.after(10, update(task, env, rl, data, episodes))
        env.mainloop()
    ...
    def sarsa_learning(task):
        ...
        env = Maze(agentXY, goalXY, tasks[task][0], tasks[task][0])
        # self, actions, maze_env, theta, discount_rate, goal_states, debug
        rl = rlalg4(list(range(env.n_actions)))
        data = {}
        env.after(10, update(task, env, rl, data, episodes))
        env.mainloop()
    ...
    def q_learning(task):
        env = Maze(agentXY, goalXY, tasks[task][0], tasks[task][0])
        rl = rlalg5(list(range(env.n_actions)))
        data = {}
        env.after(10, update(task, env, rl, data, episodes))
        env.mainloop()
    ...
    def run(method, task_num):
        if method == 0:
            rl_brain_vi(task_num)
        elif method == 1:
            rl_brain_pi(task_num)
        elif method == 2:
    ...
    for task_num, experiment_list in experiments.items():
        for env, RL, data in experiment_list:
            print("Task {}, {} : max reward = {} medLast100={} varLast100={}".format(task_num,RL.display_name, np.max(data['global_reward']),np.median(data['global_reward'][-100:]), np.var(data['global_reward'][-100:])))
