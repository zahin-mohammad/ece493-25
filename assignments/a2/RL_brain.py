...
class RLAlgorithm:
    ...
    def __init__(self, actions, environment):
        ...
        self.action_map = {0:(0,-40,0,-40), 1:(0,40,0,40), 2:(40,0,40,0), 3:(-40,0,-40,0)}
        self.env = {}
        self.walls = {}
    ...
    def init_env(self, maze_environment):
        self.walls = set([tuple(maze_environment.canvas.coords(w)) for w in maze_environment.wallblocks])

        for x in range (0, maze_env.MAZE_W):
            for y in range (0,maze_env.MAZE_H):
                center = maze_env.origin + np.array([maze_env.UNIT * x, maze_env.UNIT*y])
                # walls are not states agent can be in
                coordinates = RLAlgorithm.get_tkInter_coords((center[0], center[1]))
                if coordinates in self.walls:
                    continue
                self.env.setdefault(coordinates, {})

                for action in self.actions:
                    next_state, possible_wall_state = self.next_state(coordinates,action)
                    reward, done, __ = maze_environment.computeReward(list(coordinates), action, list(possible_wall_state))
